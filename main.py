...
def calculate_targets_stoploss(entry_price: float, signal: str, confidence: float = 0.5):
    ...
    if signal in ["BUY", "STRONG_BUY", "WEAK_BUY", "BULLISH_BOS", "QM_POTENTIAL_BUY", "RANGE_BREAKOUT_UP"]:
    ...
    elif signal in ["SELL", "STRONG_SELL", "WEAK_SELL", "BEARISH_BOS", "QM_POTENTIAL_SELL", "RANGE_BREAKOUT_DOWN"]:
...
async def get_market(symbol: str):
    """دریافت داده‌های ساده بازار"""
    try:
        # فراخوانی تابع جدید از utils.py
        data = utils.get_market_data_simple(symbol) 
...
async def get_trend_analysis(symbol: str):
    """تحلیل روند کلی"""
    try:
        # تحلیل چند تایم‌فریم برای تشخیص روند
        ...
        # دریافت قیمت فعلی
        market_info = utils.get_market_data_simple(symbol)
        
        return {
            "symbol": symbol,
            "overall_trend": overall_trend,
            "trends_by_timeframe": dict(zip(timeframes, trends)),
            "current_price": market_info.get("price", 0) if isinstance(market_info, dict) else 0,
            "analysis_time": datetime.now().isoformat()
        }
        
    except Exception as e:
        logger.error(f"خطای تحلیل روند: {e}")
        raise HTTPException(
            status_code=500,
            detail=f"خطای تحلیل روند: {str(e)[:200]}"
        )
